import rawCodes from '../RAG/Codes_by_class.json';
...
function loadCodes() {
  ...
  for (const entry of rawCodes) {
    ...
    if (!existing || entry.amount > existing.amount) byCode.set(entry.code, entry);
  }
  codesCache = [...byCode.values()];
  return codesCache;
}
...
function complaintBoost(code, docText) {
  ...
  const rules = [
    [/chest\s*pain|cardiac|acs|angina|troponin|mi\b|stemi|nstemi/i, () => {
      ...
      return h.includes('cardiovascular') || h.includes('comprehensive') ? 8 : 0;
    }],
...
function scoreCode(code, docTokens, docText, timeSlot) {
  let score = 0;
...
export function matchCodesFromText(text, limit = 12, timeSlot) {
  if (!text?.trim()) return [];
  const docTokens = new Set(tokens(text));
  return loadCodes()
    .map((c) => ({
      ...c,
      score: scoreCode(c, docTokens, text, timeSlot),
